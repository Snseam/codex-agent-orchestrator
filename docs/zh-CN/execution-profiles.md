...
Execution profile 让 CAO 在不改写全局 provider 配置的前提下，为任务选择具体的原生 agent、模型和上游端点。一个 profile 描述一个 agent、协议、endpoint、模型、凭证引用、来源信息和容量提示。任务启动时，CAO 创建本地 relay gateway，写入该 attempt 私有的运行配置，通过 Herdr 启动原生 CLI，并在 worker 停止后清理 CAO 拥有的临时文件。

旧任务仍可使用。如果任务没有 `execution` selector，且没有 default profile，CAO 会按原始流程用任务中的 `agent` 继承原生配置启动。如果设置了 default profile，CAO 会把它用于旧任务，但要求它与任务 agent 兼容。只有带显式 `execution` selector 或存在 default profile 时，才应使用 `agent: "auto"`。

## 环境要求

Profiled execution 使用 CAO 的基础要求，并需要你想驱动的原生 CLI：
...
CAO 不安装原生 CLI 或 provider。它只生成本地配置，使原生 CLI 连接 CAO 的 relay gateway。

## Profile 结构

profile 以 JSON 保存。`profile put` 会添加 CAO 控制的 revision 和时间戳；`profile export` 会移除这些元数据，使导出文件可再次导入。
...
| `capabilities` | 人工标签，供 `requireCapabilities` 使用；CAO 不从 provider 自动推断。 |
...
| `account.maxParallel` | 对共享 `account.id` 的 profile，或未设置 account 时同 endpoint host 的 CAO attempt 进行并发预约限制。它不限制原生 child agent 或单个 attempt 内的 API 请求数。 |
...
每次读取 profile 记录时，CAO 都会校验存储 schema、revision、规范化内容和 secret-like 字段。未知 secret 字段、无效 revision、非规范 JSON、不安全 env 名、symlink secret 文件和不能放进 header 的 secret 值都会被拒绝。
...
命令会去掉一个最终换行，将值保存到 CAO state directory 中的私有文件，并返回类似 `stored:anthropic-main` 的引用。secret 为空、包含控制字符或超过 64 KiB 时会被拒绝。也可以使用环境变量引用：
...
如果 `agent` 是具体值，路由只接受相同 agent 的 profile。如果 `agent` 是 `auto`，CAO 可从所有兼容候选中选择。default profile 会应用到旧式任务，但旧任务仍有具体 agent，因此 default 必须匹配该 agent，除非任务使用 `agent: "auto"`。
...
路由结果包含候选、分数、原因、选中的 profile id 和当前 CAO attempt reservation。`quota_exhausted`、`credential_unavailable`、`agent_mismatch`、`protocol_incompatible`、`shared_source_requires_allowShared` 等原因只描述路由资格。

## Gateway 生命周期

profiled task 会自动启动本地 CAO gateway。也可以手动管理：
...
gateway 是同协议 relay，支持 Anthropic Messages、OpenAI Responses 和 OpenAI Chat Completions 路径。它按 `modelMap` 或 `profile.model` 重写请求模型，并转发到同协议 profile。它不做 OAuth，不做 Anthropic/OpenAI 协议互转，也不隐藏协议不兼容。若要使用外部 gateway，请在 profile 中配置兼容的 endpoint 与 protocol；CAO 仍把它当作上游 relay target。

fallback 采用保守策略。CAO 可在明确未发出请求的网络错误，或上游返回 429/5xx 时尝试另一个 profile。如果请求字节可能已经到达上游，随后连接断开，CAO 会把结果视为不确定，而不是假定可安全切换到另一个 provider 重试。
...
CAO 为每个 attempt 写入私有文件，并传递原生 CLI 参数，使 agent 指向本地 gateway：
...
worker 停止后，CAO 清理未改变的私有执行文件。若文件被修改、替换或 owner 标记变化，CAO 会保留并记录清理证据。Gateway 日志和 attempt 证据保留在 state directory。

路由和容量限制是 CAO attempt 级控制。`account.maxParallel` 限制同 bucket 中同时活动的 CAO attempt 数量，不限制原生 child agent、provider 侧并发、HTTP 连接数或单个 attempt 内的 API 请求。
