...
Calibration makes real provider calls on cache misses. There is no automatic test on mode activation. Select at most two resources per invocation. Quick probes cap each resource at 30 seconds and the total budget at 60 seconds; code probes have separately bounded, configurable limits. Runs are serialized locally and known profile capacity is shared with normal CAO attempts.

Current probe adapters support Claude and Pi with explicit API authentication or supported CAO profiles. They do not copy OAuth credentials. An unsupported isolated authentication path is `unavailable`; it does not mean the normal native session cannot run.
...
Resource/calibration commands are usable now. They do not yet change task selection automatically: task handoff, adaptive routing and default-workflow rollout are the next slices.
