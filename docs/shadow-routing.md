...
Strategy preferences use mode schema version 2 so older clients reject unsupported state instead of silently ignoring the setting. Version-1 records stay readable and delegated. `strategy: adaptive` is not enabled in this slice. To execute in the current conversation by explicit choice, use the [host workflow](host-work.md).
