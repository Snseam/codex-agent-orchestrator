...
默认模式。每个 attempt 在状态目录下创建自己的 Git worktree，worker 只在该 worktree 中改动。验证通过后生成 `candidate.patch`；`integrate` 再把 patch 应用到目标项目。
...
`integrate` 只处理 `accepted` 且 worker 已关闭的 `worktree` attempt。它会：
