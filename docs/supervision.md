...
Foreground supervision is the first optimization slice. [Resource discovery and isolated calibration](resources.md) are also available; adaptive task selection, the current-Codex host lane and Pi RPC remain separate follow-up work. No measured speedup is claimed from protocol tests.
